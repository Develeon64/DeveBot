...
public class RulesCommand : InteractionModuleBase<SocketInteractionContext> {
	...
	public async Task AddRule ([Summary("paragraph", "The text of the new paragraph")] string paragraph) {
		if (await this.IsActivated()) {
			...
			DiscordBot.Database.Insert("rules", new Dictionary<string, object> {{"guild", this.Context.Guild.Id}, {"paragraph", paragraph}});
	...
	public async Task EditRule ([Summary("number", "Which rule to edit")] int number, [Summary("paragraph", "The text of the new paragraph")] string paragraph) {
		if (await this.IsActivated()) {
			...
			DataRowCollection? rows = DiscordBot.Database.Select("rules", null, null, null, new Expr("guild", OperatorEnum.Equals, this.Context.Guild.Id)).Rows;
			...
			DiscordBot.Database.Update("rules", new Dictionary<string, object> {{"paragraph", paragraph}}, new Expr("id", OperatorEnum.Equals, rows[number - 1].ItemArray[0]));
			await this.SendRules();
	...
	public async Task EditRule ([Summary("number", "Which rule to delete")] int number) {
		if (await this.IsActivated()) {
			...
			DiscordBot.Database.Delete("rules", new Expr("id", OperatorEnum.Equals, rows[number - 1].ItemArray[0]));
			await this.SendRules();
	...
	public class RuleSettings : InteractionModuleBase<SocketInteractionContext> {
		...
		public async Task SetRole (SocketRole role) {
			await this.DeferAsync(true);
			DiscordBot.Database.Update("guilds", new Dictionary<string, object> {{"rules_role", role.Id}}, new Expr("id", OperatorEnum.Equals, this.Context.Guild.Id));
	...
	private async Task SendRules () {
		...
		await this.Context.Guild.RulesChannel.DeleteMessagesAsync(messageIds, new RequestOptions {AuditLogReason = "New Rule added"});
	...
	private async Task<bool> IsActivated () {
		try {
			_ = (long?)DiscordBot.Database.Select("guilds", null, null, null, new Expr("id", OperatorEnum.Equals, this.Context.Guild.Id)).Rows[0].ItemArray[3] is not null;
